...
当初は句カードの表示数をブラウザの広げた幅まで柔軟に増やしたり減らすようなことを考えておりましたが、実際に作ってみるとレイアウトが崩れたり、見た目が悪くなる（大きな空白が出る場所が発生する）ため制限を加えました
...
また３つのカラムページが中央に配置されるようにするため最初は JavaScript 側で制御構文を書いていましたが、どうしても気になる挙動が出たため最終的には CSS 側で調整しました（意外にあっさりできたので最初から CSS で書いておけばよかったと後悔しています）
...
```mermaid
flowchart TD
 新規登録 -->|ユーザーの状態<新規登録>をStateコレクションに追加| 確認メール送信 --> メール内のリンクをクリックして有効化 --> |Stateからユーザーを削除|トークン発行 --> 自動ログイン
...
```mermaid
flowchart TD
パスワードを忘れましたか --> メールアドレスを入力 -->|メールアドレスがあれば送信　Stateにユーザーの状態を追加|メール内のリンクをクリック --> パスワード再設定画面 --> パスワードを入力 --> トークン発行 --> ログイン
